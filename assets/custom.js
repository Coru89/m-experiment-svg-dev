...
/* swipe event listener that shows and hides carousel arrows based on swiping and scroll position */
const scrollableElements = document.querySelectorAll('.product-list__inner--scroller');

scrollableElements.forEach(scrollableElement => {
    scrollableElement.addEventListener('touchmove', (e) => {
        const arrowEl = e.currentTarget.parentElement.nextSibling;

        if (!arrowEl) return;

        const nextButton = arrowEl.querySelector('.prev-next-button--next');
        const prevButton = arrowEl.querySelector('.prev-next-button--prev');

        const maxScrollLeft = scrollableElement.scrollWidth - scrollableElement.clientWidth;

        if (e.currentTarget.scrollLeft === 0) {
            prevButton.setAttribute('disabled', 'true');
        } else if (e.currentTarget.scrollLeft === maxScrollLeft) {
